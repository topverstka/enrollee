document.addEventListener("DOMContentLoaded", () => {
  ...
  if (document.querySelector(".filter__more_mobile")) {
  ...
  if (document.querySelector(".filter__more")) {
    filterAccordion();
  }
  ...
  function tabs() {
    ...
    for (let i = 0; i < tabBtnElems.length; i++) {
      ...
      tabBtn.addEventListener("click", () => {
        ...
        const tabBody = tabParent.querySelector(
          `[data-tabs-body="${tabData}"]`
        );
  ...
  function newPass() {
    ...
    const conArray = ["_bad", "_good", "_perfect"];
  ...
  function signupLangMore() {
    const btnElems = document.querySelectorAll(
      ".signup__checkbox-list-lang .signup__checkbox"
    );
  ...
  if (document.querySelector(".add-tag__tag-list")) {
    addTags();
  }
  function addTags() {
    ...
    function addTag(e) {
      e.preventDefault();
      if (!textarea.value == "") {
        const tagClasses = tagsBlock
          .querySelector(".add-tag__tag")
          .getAttribute("class")
          .split(" ");
    ...
    function valueNotEmpty() {
      if (!document.querySelector(".signup-help__add-textarea-error")) {
        const textareaBlock = document.querySelector(
          ".signup-help__add-textarea"
        );
        const error = document.createElement("span");
        error.classList.add("signup-help__add-textarea-error");
        error.innerText = "Поле не должно быть пустым";

        const addedError = textareaBlock.insertBefore(
          error,
          document.querySelector(".signup-help__add-textarea-btn").nextSibling
        );
  ...
  if (document.querySelector(".student-profile-how__list")) {
  ...
  function editTagsStudentProfile() {
    ...
    const btnDelete = document.querySelector(
      ".student-about__list-item-delete"
    );
    ...
    function addTag(e) {
      ...
      if (!textarea.value == "") {
        ...
        tagsBlock.insertBefore(
          tag,
          tagsBlock.querySelector(".student-about__list-item-delete")
        );

        textarea.value = "";

        if (
          !tagsBlock.querySelectorAll(".student-about__list-item").length == 0
        ) {
    ...
    tagsBlock.addEventListener("click", (e) => {
      ...
      if (target.classList.contains("student-about__list-item-delete")) {
        const tagElems = tagsBlock.querySelectorAll(
          ".student-about__list-item"
        );
      ...
      if (
        target.classList.contains("student-about__list-item") &&
        target.classList.contains("_tag-delete")
      ) {
        e.preventDefault();
        target.remove();
  ...
  function multiselect() {
    ...
    function multiselectSelectedText(selected) {
      const itemElems = document.querySelectorAll(
        ".multiselect__item._selected"
      );
